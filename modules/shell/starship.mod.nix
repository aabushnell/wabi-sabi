...
in
{
  flake.modules.nixos.starship = system;
  ...
  flake.modules.home.starship =
    ...
    let
      ...
      '';
      starshipInitNu = pkgs.runCommand "starship-init.nu" { } ''
    ...
    rc.zsh = mkIf (options ? rc.zsh)
        (mkOrder mylib.shell.rcOrder.tools "source ${starshipInitZsh}");
    rc.nu = mkIf (options ? rc.nu)
        (mkOrder mylib.shell.rcOrder.tools "source ${starshipInitNu}");
    };
}
